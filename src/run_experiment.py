...
def run_tube_loss_experiment(config, data=None):
    """
    Run a Tube Loss experiment with the given configuration.
    ...
    data : tuple, optional
        Tuple of (X, y) data. If None, loads from dataset_name in config
        
    Returns
    -------
    tuple
        (validation_objective, test_results, validation_results)
    """
    ...
    # Create Tube Loss model with specified parameters
    r = config.get("r", 0.5)
    delta = config.get("delta", 0.03)
    model = TubeLossModel(
    ...
    logger = TensorBoardLogger(save_dir="results/logs/", name=f'''TUBE_{config["dataset_name"]}_{config["random_seed"]}_{date}''')
    
    logger.log_hyperparams(config)
    
    checkpoint_callback = ModelCheckpoint(dirpath=f'''results/logs/TUBE_{config["dataset_name"]}_{config["random_seed"]}_{date}''', save_top_k=1, monitor="val_objective", filename="best_checkpoint")
